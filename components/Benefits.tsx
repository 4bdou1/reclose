...
const BenefitCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      {/* Main Card Content */}
      <div className="relative z-10 m-[2px] p-8 rounded-[calc(2rem-2px)] bg-slate-50 border border-slate-200 transition-all duration-500 group-hover:bg-white group-hover:-translate-y-4 group-hover:shadow-[0_30px_70px_-15px_rgba(56,189,248,0.3)] flex flex-col h-full overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col h-full">
          <div className="mb-8 w-14 h-14 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-sky-500 group-hover:text-white group-hover:bg-sky-500 transition-all duration-500 shadow-xl group-hover:shadow-sky-500/20">
            {icon}
          </div>

          <h4 className="text-xl font-bold text-slate-900 mb-4 tracking-wide group-hover:text-sky-500 transition-colors duration-300 font-tech uppercase">
            {title}
          </h4>
          ...
          <div className="mt-auto flex items-center gap-3 opacity-60 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0">
            <span className="text-[9px] font-tech text-sky-400 uppercase tracking-[0.2em] font-bold">Initialize {targetModule} Engine</span>
          </div>
        </div>
      </div>
    </div>
...
const Benefits: React.FC = () => {
  ...
  return (
    <section id="benefits" className="py-32 relative overflow-hidden bg-[#FAFAFA]">
      {/* Background Ambience */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center text-center mb-24">
          <Reveal>
            <span className="font-tech text-sky-500 uppercase tracking-[0.2em] text-[10px] block mb-6 font-bold">STRATEGIC EDGE</span>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-wider mb-8 leading-tight text-slate-900 font-tech uppercase">
              Scale Without <br /><span className="text-slate-500">Friction.</span>
            </h2>
            <p className="max-w-2xl text-slate-600 text-lg leading-relaxed">
              Traditional lead generation is dead. <span className="text-slate-900 font-medium">reclose</span> provides the clinical antidote:
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <Reveal delay={500} width="100%">
            ...
            >
              ...
              <h4 className="text-xl font-bold text-slate-900 mb-3 uppercase tracking-widest">Awaiting Command</h4>
        ...
        <div className="mt-32 pt-16 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-12">
          <div className="flex gap-16">
            <div className="flex flex-col">
              <span className="text-4xl font-mono font-bold text-slate-900 tracking-tighter">10X</span>
              <span className="text-[10px] text-slate-500 uppercase font-mono tracking-[0.3em] mt-2">Pipeline Velocity</span>
            </div>
            <div className="flex flex-col">
              <span className="text-4xl font-mono font-bold text-slate-900 tracking-tighter">0</span>
              <span className="text-[10px] text-slate-500 uppercase font-mono tracking-[0.3em] mt-2">Ghost Responses</span>
            </div>
          </div>
          <div className="p-5 px-8 bg-slate-100/50 border border-slate-200 rounded-2xl flex items-center gap-4 cyber-glass-light">
            <div className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse shadow-[0_0_10px_#22c55e]"></div>
            <span className="text-[10px] font-mono text-slate-600 uppercase tracking-[0.4em] font-bold">Clinical Infrastructure Online // v4.2.0</span>
          </div>
        </div>
      </div>
    </section>
